...
/// 家庭预设头像（与网页端 FAMILY_PRESETS 一致）
const _familyPresets = [
...
class _ProfileScreenState extends State<ProfileScreen> {
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: () async {
                    ...
                    try {
                      // 服务端直接持久化头像，成功后刷新即可
                      await widget.api.uploadAvatar(imagePath: picked.path);
                      if (!mounted) return;
                      ScaffoldMessenger.of(context)
                    ...
                    InkWell(
                      ...
                      onTap: () async {
                        ...
                        try {
                          await widget.api.updateProfile(
                            displayName:
                                _profile?.displayName ?? _profile?.nickname ?? '',
                            avatar: p.key,
                          );
                          if (!mounted) return;
                          ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('头像已设为${p.name}')));
                          _load();
                        } catch (_) {
                          if (!mounted) return;
                          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _editAvoidTags() async {
    final current = List<String>.from(_profile?.avoidTags ?? const []);
    final controller = TextEditingController();
    final result = await showDialog<List<String>>(
      context: context,
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialogState) => AlertDialog(
          title: const Text('我的忌口'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('选中的忌口在随机推荐时自动排除',
                    style: AppTypography.caption.copyWith(
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    for (final tag
                        in current.where((t) => !dietTagPresets.contains(t)))
                      InputChip(
                        label: Text(tag),
                        onDeleted: () =>
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('忌口已更新')));
      _load();
    } on ApiException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context)
  ...
  Future<void> _editCarouselLimit() async {
    int value = _profile?.carouselLimit ?? 10;
    if (value < 5) value = 5;
  ...
  Widget _buildBody() {
    ...
    final preset = _familyPresets.where((p) => p.key == avatar).toList();
    return ListView(
      children: [
        Padding(
              ...
              GestureDetector(
                ...
                child: preset.isNotEmpty
                    ...
                    : UserAvatar(
                        url: avatar.isEmpty
                            ? null
                            : widget.api.resolveMediaUrl(avatar),
        ...
        ListTile(
          ...
          title: const Text('家里剩啥'),
          subtitle: const Text('录入现有食材，推荐时可用'),
          trailing: const Icon(Icons.chevron_right),
          onTap: () {
            Navigator.of(context).push(
              MaterialPageRoute(
                  builder: (_) => InventoryScreen(api: widget.api)),
            );
          },
        ),
        ListTile(
        ...
        ListTile(
          ...
          title: const Text('我的忌口'),
          subtitle: Text(
            (profile.avoidTags.isEmpty)
                ? '未设置（随机推荐时自动排除）'
                : profile.avoidTags.join('、'),
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
          trailing: const Icon(Icons.chevron_right),
          onTap: _editAvoidTags,
        ),
        ListTile(
