...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        baseUrl = normalizeUrl(getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                .getString(KEY_BASE_URL, ""));

        // 首次使用：先去配置页填/扫服务器地址
        if (baseUrl.isEmpty()) {
            startActivityForResult(new Intent(this, ConfigActivity.class), 1001);
            return;
        }
    ...
    private void initWebView() {
        ...
        // 附件下载：App 内直接拉取（国内网络验证不过时系统下载器会永远排队），
        // 存到系统「下载」目录
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 1001) {
            // 配置页返回：重新读取地址，成功则初始化 WebView，否则退出
            baseUrl = normalizeUrl(getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                    .getString(KEY_BASE_URL, ""));
            if (resultCode == RESULT_OK && !baseUrl.isEmpty()) {
                initWebView();
            } else {
                finish();
            }
    ...
    private void downloadApk(final String path) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                dir.mkdirs();
                final File out = new File(dir, "whateat.apk");
                try {
                    String full = path.startsWith("http") ? path : baseUrl + path;
    ...
    private void installApk(File apk) {
        ...
        try {
            ...
            Toast.makeText(this,
                    "无法打开安装程序：请在系统设置→应用→今天吃点啥→允许「安装未知应用」后重试",
                    ...
                    public void run() {
                        ...
                        if (success) {
                            if (mediaRow != null) {
                                ...
                                getContentResolver().update(mediaRow, cv, null, null);
                            }
                            Toast.makeText(MainActivity.this,
                                    "下载完成：" + name + "（已存到系统下载）",
                                    Toast.LENGTH_LONG).show();
    ...
    private void saveCookies() {
        ...
        getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                .edit().putString("cookies", cookies).apply();
    ...
    private void restoreCookies() {
        ...
        for (String pair : cookies.split(";")) {
            String p = pair.trim();
            if (p.contains("=")) {
                CookieManager.getInstance().setCookie(baseUrl, p);
            }
        }
        CookieManager.getInstance().flush();
    }
